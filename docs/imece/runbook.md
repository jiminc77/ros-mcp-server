...
- The runner uses fresh non-interactive Gemini turns plus `--resume` for continuation. If a `T3` interrupt becomes due while a turn is still executing, the interrupt is queued and sent immediately after that turn ends.
