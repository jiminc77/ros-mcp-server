...
class DroneMCPBridge(Node):
    def __init__(self):
        ...
        self.timer = self.create_timer(
            0.05, self.timer_callback, callback_group=self.callback_group
        )
    ...
    def local_cb(self, msg):
        ...
        if not self.is_primed:
            ...
            self.get_logger().info(
                f"Ground coordinates locked: {msg.pose.position.x:.2f}, {msg.pose.position.y:.2f}"
            )
    ...
    async def execute_trajectory(self, goal_handle):
        ...
        for i, (tx, ty, tz) in enumerate(points):
            ...
            while rclpy.ok():
                ...
                drone_dist_to_wp = math.sqrt(
                    (target_x - cx) ** 2 + (target_y - cy) ** 2 + (target_z - cz) ** 2
                )
